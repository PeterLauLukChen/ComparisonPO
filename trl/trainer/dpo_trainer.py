...
class DPOTrainer(Trainer):
    ...
    def dpo_loss(
        ...
        if epsilon > 0:
            mask = (logratios.abs() > epsilon)
            chosen_logps = chosen_logps[mask]
            rejected_logps = rejected_logps[mask]
            ref_chosen_logps = ref_chosen_logps[mask]
            ref_rejected_logps = ref_rejected_logps[mask]
            logratios = logratios[mask]
            ref_logratios = ref_logratios[mask]
